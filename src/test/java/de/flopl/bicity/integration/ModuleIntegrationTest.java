package de.flopl.bicity.integration;
...
public class ModuleIntegrationTest extends TestVerticle {
	...
	public void testLogging() {
		container.logger().info("in testLogging()");
		HttpClientRequest httpClientRequest = vertx.createHttpClient()
				.setHost("localhost").setPort(8079)
				.post("/service/logging", new Handler<HttpClientResponse>() {
		...
		httpClientRequest.headers().add("guid", "adfab");
		httpClientRequest.headers().add("user", "vertx-test");
		httpClientRequest.headers().add("service", "integration-test");

		JsonObject json = new JsonObject();
		json.putString("message", "Dies ist die Lognachricht");

		httpClientRequest.end(json.toString());
	}
	...
	public void start() {
		...
		container.logger().info(
				"vertx.modulename: " + System.getProperty("vertx.modulename"));
